...
router.post('/', upload.single('image'), async (req, res) => {
    ...
    if (req.file) {
        ...
        // const buffer = await sharp(req.body.image.buffer).resize({ width: 250, height: 250 }).png().toBuffer()
...
router.put('/update/:id', auth, async (req, res) => {
    ...
    const allowedUpdates = ['title', 'companyName', 'description', 'exData', 'price', 'quantity', 'image']
...
router.put('/update/admin/:id', async (req, res) => {
    ...
    const { id } = req.params
    const medicne = await Medicne.findOne({ _id: id})
    try {
        updates.forEach(update => medicne[update] = req.body[update])
        await medicne.save()
        res.status(200).json(medicne)
...
router.delete('/delete/:id', async(req, res)=>{
    const {id} = req.params
    const item = await Medicne.findByIdAndRemove(id)
    res.json(item)
})
